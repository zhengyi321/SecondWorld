...
public class MessageQueryDialog extends Dialog {
    ...
    public MessageQueryDialog(Context context1) {
        super(context1);
        context = context1;
//        init(context);
//        setPopupWindow();
    }
    public MessageQueryDialog(Context context1, int themeResId) {
    ...
    public  class Builder {
        ...
        public Builder(Context mContext) {
         ...
         MessageQueryDialog praiseDialog;
         ...
         * 4，显示dialog的布局
         */
        public MessageQueryDialog build(Context context, String title) {
            activity = (Activity)context;
            LayoutInflater mInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            praiseDialog = new MessageQueryDialog(context, R.style.MyDialogStyle);//默认调用带style的构造
//            praiseDialog.setCanceledOnTouchOutside(false);//默认点击布局外不能取消dialog


            praiseDialog.setCancelable(true);
            View view = mInflater.inflate(R.layout.dialog_query, null);
