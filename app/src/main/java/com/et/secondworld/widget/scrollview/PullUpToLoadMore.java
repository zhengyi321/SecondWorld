...
//    MyScrollView topScrollView, bottomScrollView;
    BounceZoomScrollView topScrollView;
    ...
    private void init() {
        ...
        post(new Runnable() {
            @Override
            public void run() {
                topScrollView = (BounceZoomScrollView) getChildAt(0);
                bottomScrollView = (MyScrollView) getChildAt(1);
                topScrollView.setScrollListener(new BounceZoomScrollView.ScrollListener() {
                    @Override
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        int y = (int) ev.getY();
        Log.d("pulluptoloadmore","onInterceptTouchEvent");
        switch (ev.getAction()) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        Log.d("pulluptoloadmore","onTouchEvent");
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (currPosition == 0) {
                    if (yVelocity < 0 && yVelocity < -speed) {
                        ...
                        currPosition = 1;
                        Log.d("pulluptoloadmore","step2");
                    } else {
                        Log.d("pulluptoloadmore","step3");
                        smoothScroll(0);
                    }
                    Log.d("pulluptoloadmore","step4");
                } else {
                    Log.d("pulluptoloadmore","step5");
                    if (yVelocity > 0 && yVelocity > speed) {
                        Log.d("pulluptoloadmore","step6");
                        smoothScroll(0);
                        currPosition = 0;
                    } else {
                        smoothScroll(position1Y);
                        Log.d("pulluptoloadmore","step7");
                    }
                    Log.d("pulluptoloadmore","step8");
                }
                break;
        }
        lastY = y;
