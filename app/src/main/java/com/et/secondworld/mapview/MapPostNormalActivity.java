...
import com.et.secondworld.forum.ForumAtActivity;
 ...
 * @Date 2020/6/20
 **/
public class MapPostNormalActivity  extends AppCompatActivity implements LocationSource, TencentLocationListener {
    ...
    REditText retForumPostContent;
    @OnClick(R.id.rly_forum_post_loc_close)
    public void ibForumPostLocCloseOnclick(){
        addr = "";
        llyForumPostLoc.setVisibility(View.GONE);
    }
...
//            handler.sendMessage(msg);
    ...
    public void rlyForumPostContentOnclick(){
        retForumPostContent.requestFocus();
        InputMethodManager imm = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
        imm.toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS);
    }
...
//        release();
    ...
    private void intoExtendActivity(){
        ...
        List<RObject> rObjectList = retForumPostContent.getObjects();
...
//        intent.putExtra("type","M4");
       /* switch (type){
        ...
        startActivityForResult(intent,EXTEND_OPEN);
    }
    /**
     * 添加设置话题
     *
...
//        startActivityForResult(intent, SELECT_AT_REQUEST);
    ...
    private void init(){
        ...
        llyForumPostLoc.setVisibility(View.GONE);
        rlyForumPostLoc.setVisibility(View.GONE);
...
//                                etForumPostContent.setObject(rObject);
                        ...
                        retForumPostContent.setText(content.trim());
                        etForumPostTitle.setText(title);
                        if(!addr.isEmpty()){
                            tvForumPostLoc.setText(addr);
                        }
                        for(RObject item:rObjectList){
                            retForumPostContent.setObject(item);
                        }
                    }catch (Exception e){
...
//                            Toast.makeText(this,""+item.getId(),Toast.LENGTH_LONG).show();
                ...
                case EXTEND_OPEN:{
                    ...
                    intent.putExtra("lat", lat);
                    intent.putExtra("lon", lon);
                    intent.putExtra("title", etForumPostTitle.getText().toString());
                    //设置返回数据
        ...
        int error = TencentLocationManager.getInstance(this)
                .requestLocationUpdates(
                        TencentLocationRequest
                                ...
                                .setRequestLevel(
                                        TencentLocationRequest.REQUEST_LEVEL_POI), this);
...
//            village = location.getTown();
            Log.d("locationmap11",province+" "+city+" "+village +location.getName()+" "+location.getNation());
