package com.et.secondworld.huanxin.widget;
...
import com.et.secondworld.huanxin.model.EaseVoiceRecorder;
import com.et.secondworld.huanxin.utils.EaseCommonUtils;
import com.et.secondworld.huanxin.widget.chatrow.EaseChatRowVoicePlayer;
    ...
    public boolean onPressToSpeakBtnTouch(View v, MotionEvent event, EaseVoiceRecorderCallback recorderCallback) {
        ...
        case MotionEvent.ACTION_DOWN:
            try {
                ...
                v.setPressed(true);
                //进行权限的判断
                /*boolean hasPermission = PermissionUtil.checkPermission(v.getContext());
                if (!hasPermission) {
                    ActivityCompat.requestPermissions((Activity) v.getContext(), new String[]{Manifest.permission.CAMERA, Manifest.permission.RECORD_AUDIO},0x00);
                } else {
                    startRecording();
                }*/
                // 判断环境兼容，检查自己的权限，是否被同意
                if (ContextCompat.checkSelfPermission((Activity) v.getContext(), Manifest.permission.RECORD_AUDIO)!= PackageManager.PERMISSION_GRANTED){
                    //如果不同意，就去请求权限   参数1：上下文，2：权限，3：请求码
                    ActivityCompat.requestPermissions((Activity) v.getContext(),new String []{Manifest.permission.RECORD_AUDIO},1);
                }else {
                    //同意就拨打
                    startRecording();
                }
